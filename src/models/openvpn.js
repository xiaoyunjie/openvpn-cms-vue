...
class OpenVPN {
  ...
  // 历史信息搜索
  async searchkeyworkinfo({ count = this.uCount, page = this.uPage, name, keyword, start, end }) {
    try {
      const res = await get('v1/openvpn/infosearch', { count, page, name, keyword, start, end }, { handleError: true })
      return res
    } catch (error) {
      console.log(error)
    }
