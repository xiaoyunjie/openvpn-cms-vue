<template>
  <div class="lin-table">
    ...
    >
      ...
      <el-table-column v-if="operate.length > 0" label="操作" fixed="right" width="400">
        <template slot-scope="scope">
          <el-button
...
export default {
  ...
  methods: {
    ...
    // 行内更新
    handleupdate(_this, index, row) {
      _this.$emit('handleupdate', { index, row })
    },
    // 行内查询
    handlesearchip(_this, index, row) {
      _this.$emit('handlesearchip', { index, row })
    },
    // 证书下载
    certdownload(_this, index, row) {
      _this.$emit('certdownload', { index, row })
    },
    // 多选-选中checkbox
